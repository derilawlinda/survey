...
class DataTables extends Widget
{
        ...
        foreach ($headerRows as $rowIndex => $aHeaderRow) {
            foreach ($aHeaderRow as $colIndex => $header) {
                ...
                if (! isset($currentText)) {
                    ...
                    $rowspan = (int) Util::init($newHeaderRows, 
                        [$lastNotNull, $colIndex, 'rowspan'], 1);
                    ...
                    if (! $isEqual || ! $isSameParents) {
                        ...
                        $colspan = (int) Util::init($newHeaderRows, 
                            [$rowIndex, $lastIndex, 'colspan'], 1);
    ...
    protected function getSearchAllSql($datasource, $columns, 
        $searchAllString, & $queryParams)
    {
    ...
    protected function parseRequest($datasource, $dtId, $method = 'get') 
    {
        ...
        if ($id == $dtId) {
            ...
            $searchAllSql = $this->getSearchAllSql(
                $datasource, $columns, $searchAllString, $queryParams);
